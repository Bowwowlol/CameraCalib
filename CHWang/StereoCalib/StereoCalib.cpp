...
//
void Stereo::StereoCalib(const char* imageList, int useUncalibrated)
{
    CvRect roi1, roi2;
    //int nx = 0, ny = 0;
    ...
    //FIND CHESSBOARDS AND CORNERS THEREIN:
        ...
        if( displayCorners )
        {
            ...
            cvDrawChessboardCorners( cimg, cvSize(nx, ny), &temp[0],
                count, result );
            IplImage* cimg1 = cvCreateImage(cvSize(320, 240), IPL_DEPTH_8U, 3);
            cvResize(cimg, cimg1);
			...
			char *path;
			path = _getcwd(NULL, 0);
			sprintf_s(str, "%s\\photo\\photo", path);
			_mkdir(str);
	
			sprintf_s(STR, "%s\\%.2d.bmp", str,nframes);

			cvSaveImage(STR,cimg1);
            cvReleaseImage( &cimg );
...
// CALIBRATE THE STEREO CAMERAS
   // printf("Running stereo calibration ...");
	...
	cvCalibrateCamera2( &_objectPoints, &_imagePoints1, &_npoints, imageSize, &_M1, &_D1, NULL, NULL, CV_CALIB_FIX_K3);//CV_CALIB_FIX_ASPECT_RATIO
	cvCalibrateCamera2( &_objectPoints, &_imagePoints2, &_npoints, imageSize, &_M2, &_D2, NULL, NULL, CV_CALIB_FIX_K3);//CV_CALIB_FIX_ASPECT_RATIO
...
//Always work in undistorted space
	...
	fstream Data("Camera.txt", ios::app);
			Data << "l_u0" << setw(40) << cvmGet( &_M1, 0, 2 ) << endl << "l_v0" << setw(40) << cvmGet( &_M1, 0, 5 ) <<endl;
			Data << "l_fu" << setw(40) << cvmGet( &_M1, 0, 0 ) << endl << "l_fv" << setw(40) << cvmGet( &_M1, 0, 4 ) <<endl;
			
	
			Ima.l_u0=cvmGet( &_M1, 0, 2 );
			Ima.l_v0=cvmGet( &_M1, 0, 5 );
			Ima.l_fu=cvmGet( &_M1, 0, 0 );
			Ima.l_fv=cvmGet( &_M1, 0, 4 );
			Ima.r_u0=cvmGet( &_M2, 0, 2 );
			Ima.r_v0=cvmGet( &_M2, 0, 5 );
			Ima.r_fu=cvmGet( &_M2, 0, 0 );
			Ima.r_fv=cvmGet( &_M2, 0, 4 );
			Ima.l_kc[0]=cvmGet( &_D1, 0, 0 );Ima.l_kc[1]=cvmGet( &_D1, 0, 1 );Ima.l_kc[2]=cvmGet( &_D1, 0, 2 );
			Ima.l_kc[3]=cvmGet( &_D1, 0, 3 );Ima.l_kc[4]=cvmGet( &_D1, 0, 4 );
			Ima.r_kc[0]=cvmGet( &_D2, 0, 0 );Ima.r_kc[1]=cvmGet( &_D2, 0, 1 );Ima.r_kc[2]=cvmGet( &_D2, 0, 2 );
			Ima.r_kc[3]=cvmGet( &_D2, 0, 3 );Ima.r_kc[4]=cvmGet( &_D2, 0, 4 );

			Ima.image_correction_left();

			Data << "r_u0" << setw(40) << cvmGet( &_M2, 0, 2 ) << endl << "r_v0" << setw(40) << cvmGet( &_M2, 0, 5 ) <<endl;
			Data << "r_fu" << setw(40) << cvmGet( &_M2, 0, 0 ) << endl << "r_fv" << setw(40) << cvmGet( &_M2, 0, 4 ) <<endl;

			Ima.image_correction_right();

			Data << "L" << setw(40) << abs(cvmGet( &matT, 0, 0 )/1000) << endl;
			Data.close();
...
// IF BY CALIBRATED (BOUGUET'S METHOD)
        if( useUncalibrated == 0 )
        {
			...
			cvSave("R_matrix.xml",&matR);
			cvSave("T_matrix.xml",&matT);
